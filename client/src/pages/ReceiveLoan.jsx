...
export default function ReceiveLoan() {
  ...
  const handleDeposit = async () => {
    try {
      await axios.post(`http://localhost:8000/loan/receive/${accountNumber}`, { amount });
      setAccountNumber('');
      setAmount('');
      navigate('/loan/receive');
      setSuccessMessage('Loan recevied successful!');
      setTimeout(() => {
        setSuccessMessage('');
      }, 2000);
    } catch (error) {
  ...
  return (
    <div className="container-fluid d-flex flex-column align-items-center" style={{ height: 'calc(100vh - 260px)' }}>
      <div style={{ height: '60px' }}>
        {error && (
          <div className="alert alert-warning" role="alert" style={{ width: '100vw' }}>{error}</div>
        )}
        {successMessage && (
