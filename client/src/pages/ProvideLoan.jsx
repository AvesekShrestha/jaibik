...
export default function ProvideLoan() {
  ...
  const handleProvideLoan = async () => {
    try {
      await axios.post('http://localhost:8000/loan/provide', { accountNumber, amount });
      setAccountNumber('');
      setAmount('');
      navigate('/loan/provide');
      setSuccessMessage('Loan provided successfully!');
      setTimeout(() => {
        setSuccessMessage('');
      }, 2000);
    } catch (error) {
  ...
  return (
    <div className="container-fluid d-flex flex-column align-items-center" style={{ height: 'calc(100vh - 260px)' }}>
            ...
            <div className="card-body">
              {accountHolder.map((element, index) => (
                <div key={index}>
                  ...
                  <p className="card-text">{`Previous Loan: Rs.${element.balance.loan}`}</p>
                  ...
                  <p className="card-text">{`Loan Limit: Rs.${element.type.loanLimit}`}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <form className="container mt-4">
        ...
        <button type="button" className={`btn btn-primary ${error ? 'disabled' : ''}`} onClick={handleProvideLoan}>
          Provide Loan
        </button>
      </form>
    </div>
